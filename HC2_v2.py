...
#        return rows
    ...
    def Talk(self, talk, context):
        a = {}
        ##m2u으로 시작
        talk.utter.utter = talk.utter.utter + ";$contractNo=93$call_Id=3004"
        print("talk : ", talk.utter.utter)
        #seq = talk.utter.utter[talk.utter.utter.index(";$callSeq")+10:talk.utter.utter.index(";$callSeq")+11]
        seq = talk.utter.utter[talk.utter.utter.index(";$contractNo")+13:talk.utter.utter.index("$call_")]
...
#                print("===============test=============")
        ...
        if sds_res['current_task'] == 'task1':
            if (sds_intent != 'affirm' and sds_intent != 'negate') and len(talk.utter.utter) >= 40:
...
#            if sds_intent == 'again':
#                talk_res.response.speech.utter = "고객님의 주소는 " + str(privacy_add1) + ", " + str(privacy_add2) + "로 확인 되는데 맞으십니까?"
#            elif sds_intent == 'unknown':
#                talk_res.response.speech.utter = "네, 또는 아니오로 답변해주세요,"
#            else:
...
#\            if sds_intent == 'again':
#\                talk_res.response.speech.utter = "약관과 청약서 부본을 전달받지 못 하셨거나, 자필서명을 하지 않은 경우, 그리고 중요사항을 제대로 설명받지 못한 경우에는, 청약일로부터 삼개월 이내에 계약취소를 요청하실수 있다는 내용을 알고 계십니까?"
#\            elif sds_intent == 'unknown':
#\                talk_res.response.speech.utter = "네, 또는 아니오로 답변해주세요,"
#\            else:
...
#\            if sds_intent == 'again':
#\                talk_res.response.speech.utter = "고객님께서 가입하신 담보는 "+ dambo +" 으로, 가입한 모든 담보에 대해 중복가입시 보험금을 중복해서 받으실 수 있다고 설명들으신 경우, 중복보상으로 답변을, 실제손해액을 한도로 비례보상 된다고 설명들으신 경우, 비례보상으로 답변주시기 바랍니다."
#\            elif sds_intent == 'unknown':
