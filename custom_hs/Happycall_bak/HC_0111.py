...
#            session_id = int(dbsession[0]['session_value'])
        ...
        if task == 'PRIVACY3' and sds_intent == "privacy3":
            ...
            model = 'Happy_Call_HH'
        if task == 'PRIVACY1_1':
            privacy1_1DB = self.DBConnect("select privacy1_1,privacy1_1affirm from task_logic where seq = '" + seq  + "';")
            ...
            if sds_intent == 'privacy1': 
                ...
                if cust_ssn ==  privacy1_1affirm:
                   ...
                   sds_res = self.Sds.Talk(talk.utter.utter, session_id, model)
                    ...
                    print(privacy1_1)
                    self.DBConnect("update task_logic set privacy1_1='"+str(privacy1_1)+"' where seq = '" + seq + "';")
                    sds_res = self.Sds.Talk(talk.utter.utter, session_id, model)
       ...
       # if sds_res['current_task'] == 'PRIVACY1':
        if sds_res['current_task'] == 'PRIVACY1':
            privacy1_1DB = self.DBConnect("select privacy1_1 from task_logic where seq = '" + seq  + "';")
            privacy1_1 = privacy1_1DB[0]['privacy1_1']
       ...
       # if sds_res['current_task'] == 'PRIVACY1_1':
            ...
            if sds_intent == 'privacy1':
                ...
                self.DBConnect("update task_logic set privacy1_1affirm='"+input_print+"' where seq = '" + seq + "';") 
                print("cust_ssn" + cust_ssn)
        task_list = ['task1','task2','task3','task4','task5','task6','task7','task8','task9','task9_1']
        for i in task_list:
          if sds_res['current_task'] == i:
              sds_intent = self.Sds.GetIntent(talk.utter.utter, model)
              DBcount = self.DBConnect("select "+i+" from task_logic where seq = '" + seq  + "';")
              count = DBcount[0][i]
              if int(count) >= 2:
                  camp_id_db = self.DBConnect("select camp_id from hc_hh_campaign_info where task = '" + sds_res['current_task'] + "';")
                  print (camp_id_db)
                  camp_id = str(camp_id_db[0]['camp_id'])
                  talk.utter.utter = "$"+i+"$"
                  self.DBConnect("insert into hc_hh_campaign_score (call_id, contract_no, info_seq, info_task, task_value) values ('"+call_id+"','"+seq+"','"+camp_id+"','"+sds_res['current_task'] +"','모름')");
                  sds_res = self.Sds.Talk(talk.utter.utter, session_id, model)
                  ...
                  count = DBcount[0][i]
                  count = int(count) + 1
                  self.DBConnect("update task_logic set "+i+"='"+str(count)+"' where seq = '" + seq + "';")
...
#             
#            if sds_intent == 'affirm':
#                if cust_ssn ==  privacy1_1aiffirm:
#                   talk.utter.utter = "$privacy1_1$"
#                   sds_res = self.Sds.Talk(talk.utter.utter, session_id, model)
#                else:
#                   talk.utter.utter = "$privacy1_1back$"
#                   privacy1_1 = int(privacy1_1) + 1
#                   self.DBConnect("update task_logic set privacy1_1='"+privacy1_1+"' where seq = '" + seq + "';")
#                   sds_res = self.Sds.Talk(talk.utter.utter, session_id, model)
#            if sds_intent == 'negate':
#                talk.utter.utter = "$privacy1_1back$"
#                privacy1_1 = int(privacy1_1) + 1
#                self.DBConnect("update task_logic set privacy1_1='"+privacy1_1+"' where seq = '" + seq + "';")
#                sds_res = self.Sds.Talk(talk.utter.utter, session_id, model)
...
#            slot_minute = int(slot_minute)
        ...
        if sds_res['current_task'] == 'task1':
            if sds_intent == "again":
                ...
                talk_res.response.speech.utter = "그러시군요, 말씀해주셔서 감사합니다, 현대해상 고객센터 였습니다 $callback$"
            else:
        ...
        if sds_res['current_task'] == 'task2':
            if sds_intent == 'again':
                ...
                talk_res.response.speech.utter = "네 고객님, 반갑습니다," + self.readNumberMinute(join_month) + "월" + self.readNumberMinute(join_day) +"일, 저희 현대해상 "+prod_name+"을 가입해 주셔서, 진심으로 감사드립니다, 가입하실때, 상품의 중요한 사항이 제대로 설명되었는지, 확인드리고자 연락드렸습니다, 소요시간은 약 삼분정도인데, 잠시 통화 가능하십니까?"
        #PRIVACY1
        if sds_res['current_task'] == 'PRIVACY1':
            if sds_intent == 'again':
                talk_res.response.speech.utter = "본인확인을 위해 생년월일을,  년과 월, 일로 말씀해주세요"
            elif sds_intent == 'unknown':
                talk_res.response.speech.utter = "생년월일을,  년과 월, 일로 말씀해주세요"
            elif int(privacy1_1) == 1:
                talk_res.response.speech.utter = "생년월일 정보가 일치하지 않습니다, 다시 한번 년과 월, 일로 말씀해주세요."
        ...
        if sds_res['current_task'] == 'PRIVACY1_1':
            if sds_intent == 'again':
                talk_res.response.speech.utter = "말씀하신 주민번호는 " +self.readNumberMinute(input_year)+ "년" +self.readNumberMinute(input_month)+ "월" +self.readNumberMinute(input_day)+ "일이,  맞으신가요?"
            elif sds_intent == 'unknown':
                talk_res.response.speech.utter = "네, 또는 아니오로 답변해주세요,"
            else:
                talk_res.response.speech.utter = "말씀하신 주민번호는 " +self.readNumberMinute(input_year)+ "년" +self.readNumberMinute(input_month)+ "월" +self.readNumberMinute(input_day)+ "일이, 맞으신가요?"
        ...
        if sds_res['current_task'] == 'task3':
            if sds_intent == 'again':
                talk_res.response.speech.utter = "계약하실 때 계약자 " + insured_contractor + "님께서 청약서, 상품설명서, 개인정보처리 동의서에 직접 서명하셨습니까?"
            elif sds_intent == 'unknown':
                talk_res.response.speech.utter = "네, 또는 아니오로 답변해주세요,"
            else:
        ...
        if sds_res['current_task'] == 'task4':
            if sds_intent == 'again':
                talk_res.response.speech.utter = "타인의 사망을 보장 해주는 계약의 경우 보험대상자도 반드시 서면동의를
 해주셔야 하는데요, 피보험자 "+ insured_person +" 님도 직접 서명하셨습니까?"
            elif sds_intent == 'unknown':
                talk_res.response.speech.utter = "네, 또는 아니오로 답변해주세요,"
            else:
...
#                talk_res.response.speech.utter = task8_print + "보험료 납입기간 중 중도 해지시, 해지환급금이 지급되지 않는다는 설명을 들으셨나요?"
        ...
        if sds_res['current_task'] == 'time':
            if sds_intent == 'again':
                ...
                talk_res.response.speech.utter = "다시 한번 편하신 요일과 시를, 말씀해주세요."
            else:
                talk_res.response.speech.utter = "그럼 가능하신 시간을 알려주시면 다시 연락드리겠습니다,주말도 가능하니, 편하신 요일과 시를,말씀해주세요"
        #time_end       
        if sds_res['current_task'] == 'timeEnd':
...
#                talk_res.response.speech.utter = "법률비용 담보는 중복가입시 보험금을 중복해서 받으실 수 있다고 설명 들으셨다면 중복, 실제지급액을 한도로 비례보상 된다는 설명을 들으셨다면 비례를 말씀해주세요."
        ...
        if sds_res['current_task'] == 'taskEnd':
            ...
            talk_res.response.speech.utter = "긴 시간 답변 감사드립니다, 향후 불편하시거나 궁금하신 점, 있으시면 담당자나 고객콜센터로 언제든지 연락주시기 바랍니다, 오늘도 행복한 하루 보내세요. $complete$"
