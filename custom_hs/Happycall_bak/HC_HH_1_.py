...
class EchoDa(talk_pb2_grpc.DialogAgentProviderServicer):
    ...
    def Talk(self, talk, context):
        ...
        phoneNum = self.DBConnect("select phone from hc_hh_1_score where seq_num = '" + seq + "';")
        ...
        dbsession = self.DBConnect("select session_name,phone,product_code,user_name,join_time,talk_time,insurance_contractor,insurance_insured,insurance_closeproduct,privacy_add1,privacy_add2,insurance_productname from hc_hh_1_score where seq_num = '" + seq + "';")
        # user_name 통화자(가입자)
        # join_time 전체 시간 호출
        # join_month 가입 월
        # join_day 가입 일
        ...
        if dbsession[0]['session_name'] is None:
            ...
            self.DBConnect("update hc_hh_1_score set session_name = '" + str(dbsessioncode) + "' where seq_num = '" + seq + "';")
        ...
        ##초기 모델 설정
        dbsession = self.DBConnect("select model from hc_hh_1_score where seq_num = '" + seq + "';")
        ...
        # SDS
        dbtask = self.DBConnect("select task from hc_hh_1_score where seq_num = '" + seq + "';")
        task = dbtask[0]['task']
        ...
        if task == 'task2' and sds_intent == "affirm":
            ...
            self.DBConnect("update hc_hh_1_score set model='privacy' where seq_num = '" + seq + "';")
            model = 'privacy'
        ...
        if task == 'PRIVACY2' and sds_intent == "affirm":
            ...
            self.DBConnect("update hc_hh_1_score set model='Happy_Call_HH' where seq_num = '" + seq + "';")
            model = 'Happy_Call_HH'
        ...
        #self.DBConnect("update test set task='" + sds_res['current_task'] + "' where seq_num = '" + seq + "';")
        #sds_intent = self.Sds.GetIntent(talk.utter.utter, model)
        ...
        if task in b:
            if sds_intent == 'affirm':
                self.DBConnect("update hc_hh_1_score set " + task + "='네' where seq_num = '" + seq + "';")
            elif sds_intent == 'negate':
                self.DBConnect("update hc_hh_1_score set " + task + "='아니오' where seq_num = '" + seq + "';")
            elif sds_intent == 'proportion' or sds_indent == 'nooverlap':
                self.DBConnect("update hc_hh_1_score set " + task + "='비례' where seq_num = '" + seq + "';")
            elif sds_intent == 'overlap' or sds_indent == 'noproportion':
                self.DBConnect("update hc_hh_1_score set " + task + "='중복' where seq_num = '" + seq + "';")
            elif sds_intent == 'next':
                pass
            else:
                self.DBConnect("update hc_hh_1_score set " + task + "='"+talk.utter.utter+"' where seq_num = '" + seq + "';")
        ...
        #현재 위치를 저장하는 로직
        self.DBConnect("update hc_hh_1_score set task='"+ sds_res['current_task'] + "' where seq_num = '" + seq + "';")

        print("SDS Start!")
        #dbtask = self.DBConnect("select task from test where seq_num = '" + seq + "';")
        #task = dbtask[0]['task']
        ...
        # 값을 append을 했을때, 해당 로직을 점프를 하는 것으로.

        # SDS
        dbtask = self.DBConnect("select task from hc_hh_1_score where seq_num = '" + seq + "';")
        task = dbtask[0]['task']
        if task == 'task4' and insurance_contractor == insurance_insured:
            talk.utter.utter = "$task4$"
            self.DBConnect("update hc_hh_1_score set task='task5' where seq_num = '" + seq + "';")
            sds_res = self.Sds.Talk(talk.utter.utter, session_id, model, product_code)
...
#        if product_code == 'a': sds_list.append(9,10,11,12,13,14)
        ...
        for sds_temp in sds_list:
            ...
            if task is None or task == "":
                ...
                if tasknumber == str(sds_temp):
                    ...
                    self.DBConnect("update hc_hh_1_score set task='task"+str(sds_temp+1)+"' where seq_num = '" + seq + "';")
                    sds_res = self.Sds.Talk(talk.utter.utter, session_id, model, product_code)
                    if tasknumber == 13:
                        pass
                    else:
                        ...
                        print("utter가 들어간 값은 : " + talk.utter.utter)
                        self.DBConnect("update hc_hh_1_score set task='task"+str(sds_temp+2)+"' where seq_num = '" + seq + "';")
                        sds_res = self.Sds.Talk(talk.utter.utter, session_id, model, product_code)
        ...
        if sds_res['current_task'] == 'timeAffirm':
            if talk.utter.utter == "$hourMiss$" or talk.utter.utter == "$dayMiss$":
                ...
                self.DBConnect("update hc_hh_1_score set next_time='"+str(next_time)+"' where seq_num = '" + seq + "';")
        ...
        if sds_res['current_task'] == 'task9':
            if sds_intent == "unknown":
                talk_res.response.speech.utter = "화재벌금 담보는 중복 가입시 비례보상됩니다, 화재벌금 또는 과실 치사상 벌금 담보 등은 중복가입시 보험금을 중복해서 받으실 수 있다고 설명 들으셨나요, 아니면 실제손해액을 한도로 비례보상 된다는 설명을 들으셨나요?"
        # task11
        if sds_res['current_task'] == 'task11':
            if sds_intent == "unknown":
                talk_res.response.speech.utter = "일상생활배상책임 담보는 중복 가입시 비례보상됩니다, 일상생활배상책임 담보는 중복가입시 보험금을 중복해서 받으실 수 있다고 설명 들으셨나요,아니면 실제손해액을 한도로 비례보상 된다는 설명을 들으셨나요?"
        # task13
        if sds_res['current_task'] == 'task13':
            if sds_intent == "unknown":
                talk_res.response.speech.utter = "법률비용 담보는 중복 가입시 비례보상됩니다, 법률비용 담보는 중복가입시 보험금을 중복해서 받으실 수 있다고 설명 들으셨나요, 실제지급액을 한도로 비례보상 된다는 설명을 들으셨나요?"
        # task13
