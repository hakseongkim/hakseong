...
##    def DBConnect_HC(self, query):
    ...
    def Talk(self, talk, context):
        ...
        """
        ##m2u으로 시작
     #   talk.utter.utter = talk.utter.utter + ";$callSeq=10$call_Id=2000"
       ...
       # seq = talk.utter.utter[talk.utter.utter.index(";$callSeq")+10:talk.utter.utter.index(";$callSeq")+11]
        ...
        if reset_count == '1':
            ...
            self.DBConnect("update task_logic set PRIVACY1=0,task1=0,task2=0,task3=0,task4=0,task5=0,PRIVACY1_1=0,PRIVACY2=0,privacy1_1affirm=0,time=0,timeaffirm=0 where contract_no = "+seq+";")
...
#            slot_minute = int(slot_minute)
        ...
        if sds_res['current_task'] == 'task1':
            if sds_intent == "again":
                talk_res.response.speech.utter = "현대해상 AI상담원입니다, "+ user_name + "고객님 되시나요?"
                ...
                talk_res.response.speech.utter = "안녕하십니까?, 현대해상 AI상담원입니다, " + user_name + "고객님 되십니까?"
...
#            talk_res.response.speech.utter = "말씀해주셔서 감사합니다, 고객님의 주소는 " + str(privacy_add1) + " 으로 확인되는데요, 나머지 주소는 어떻게 되십니까?"
        ...
        if sds_res['current_task'] == 'task3':
            if sds_intent == 'again':
                ...
                talk_res.response.speech.utter = "확인 감사합니다, 계약자와 피보험자가 다른 계약의 경우, 피보험자이신 "+ insured_person +" 고객님의 계약체결에 대한 동의가 반드시 필요합니다. 자필서명이 정확하게 이루어지지 않은 경우, 무효계약으로 고객님께서 불이익을 보실 수 있습니다. 고객님께서 청약서에 직접 자필서명을 하셨는지요?"
        # task4
        if sds_res['current_task'] == 'task4':
            if sds_intent == 'again':
                ...
                talk_res.response.speech.utter = "마지막 질문입니다, 직업, 건강상태 등 계약전 알릴의무 사항을 제대로 알리지 않으면 , 향후 보험금 지급이 제한될 수 있는데요, 고객님께서 해당 내용을 정확히 확인하고 작성하셨습니까?"
