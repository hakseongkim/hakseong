...
class EchoDa(talk_pb2_grpc.DialogAgentProviderServicer):
    ...
    def Talk(self, talk, context):
        ...
        dbsession = self.DBConnect("select session_value,cust_tel_no,cust_nm,join_time,talk_time,insured_contractor,insured_person,insured_closeproduct,address_main,address_sub,product_code1, product_code2, product_code3 ,prod_name from campaign_target_list_tb where contract_no = '" + seq + "';")
        # cust_nm 통화자(가입자)
        # join_time 전체 시간 호출
        # join_month 가입 월
        # join_day 가입 일
       ...
       #디비 이관 전 작업하던더
        ...
        if task == 'task9' or task == 'task9_1':
            if product_code1 == 'Y':
                dambo = dambo + "화재벌금 또는 과실치사상 벌금 담보, "
            if product_code2 == 'Y':
                dambo = dambo + "일상생활배상책임 담보, "
            if product_code3 == 'Y':
                dambo = dambo + "법률비용 담보, "
...
#            slot_minute = int(slot_minute)
        ...
        if sds_res['current_task'] == 'task9_1':
            talk_res.response.speech.utter = "가입하신 "+ dambo+"은 중복가입시 모두 비례보상됩니다. 다시 질문드리겠습니다. 담보를 중복으로 가입하셨을 경우, 실제손해액을 한도로 비례보상 된다고 설명 들으셨나요?"
        #time        
