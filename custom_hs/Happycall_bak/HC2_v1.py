...
#        DBcount = self.DBConnect("select PRIVACY2 from task_logic where contract_no = '" + seq  + "';")
#        privacy2_count = DBcount[0]['PRIVACY2']
#        if task == 'PRIVACY2' and (sds_intent == "affirm" or int(privacy2_count) >= 1):
#            print("성공22222")
#            talk.utter.utter = '$next$'
#            self.DBConnect("update campaign_target_list_tb set model='Happy_Call_HH2' where contract_no = '" + seq + "';")
#            model = 'Happy_Call_HH2'
#            unknown_count = 1
#            seq_id_db = self.DBConnect("select seq from hc_hh_campaign_info where task = '" + task + "';")
#            seq_id = str(seq_id_db[0]['seq'])
#            self.DBConnect("insert into hc_hh_campaign_score (call_id, contract_no, info_seq, info_task, task_value) values ('"+call_id+"','"+seq+"','"+seq_id+"','"     +task +"','모름')");
#
...
#            if sds_intent == 'privacy1': 
#                pass
#            elif sds_intent == 'affirm':
...
#            if input_six != '' and (input_year == '' and input_month == '' and input_day == ''):
...
#                print("cust_ssn" + cust_ssn)
  ...
  #              self.DBConnect("insert into hc_hh_campaign_score (call_id, contract_no, info_seq, info_task, task_value) values ('"+call_id+"','"+seq+"','"+seq_id+"','"+task +"','Y')")
            elif sds_intent == 'negate':
 ...
 #               self.DBConnect("insert into hc_hh_campaign_score (call_id, contract_no, info_seq, info_task, task_value) values ('"+call_id+"','"+seq+"','"+seq_id+"','"+task +"','N')")
...
#                self.DBConnect("insert into hc_hh_campaign_score (call_id, contract_no, info_seq, info_task, task_value) values ('"+call_id+"','"+seq+"','"+seq_id+"','"+task +"','중복')")    
...
#                self.DBConnect("insert into hc_hh_campaign_score (call_id, contract_no, info_seq, info_task, task_value) values ('"+call_id+"','"+seq+"','"+seq_id+"','"+task +"','비례')")    ;
...
#                self.DBConnect("insert into hc_hh_campaign_score (call_id, contract_no, info_seq, info_task, task_value) values ('"+call_id+"','"+seq+"','"+seq_id+"','"+task +"','입력')")    ;
...
#        print(sds_res)
       ...
       #시간 컨펌
        ...
        if sds_res['current_task'] == 'timeAffirm':
            ...
            print ('인풋 월 출력' + str(slot_input_month))
            re_input = str(slot_input_month)
            re_input = re_input.replace(' ','')
            re_input = re_input.replace('시','')
            print ('들어온 시는 : '+ str(re_input))
            hanCount = len(re.findall(u'[\u3130-\u318F\uAC00-\uD7A3]+', re_input.decode('utf-8')))
            print(hanCount)
            if hanCount >= 1:
                print("여기에 들어왔따")
                slot_input_month = self.change_bu(re_input.decode('utf-8'))
                hanCount = len(re.findall(u'[\u3130-\u318F\uAC00-\uD7A3]+', slot_input_month.decode('utf-8')))
                if hanCount >= 1:
                    slot_input_month = self.change(slot_input_month.decode('utf-8'))
                else:
                    print('인풋 월에서 1차 분류 거치고 한글은 더이상 없는 것 같네요')
                    pass
            else:
                print('인풋 월에서는 한글은 없는 것 같네요')
                pass
            print ('인풋 일 출력' + str(slot_input_day))
#            slot_hour = int(slot_hour)
            ### 분를 수정하는 로직
            re_input = str(slot_input_day)
            print ('인풋 일은 : '+ str(slot_input_day))
            re_input = re_input.replace(' ','')
            re_input = re_input.replace('일','')

            hanCount = len(re.findall(u'[\u3130-\u318F\uAC00-\uD7A3]+', re_input.decode('utf-8')))
            if hanCount >= 1:
                slot_input_day = self.change_bu(re_input.decode('utf-8'))
                hanCount = len(re.findall(u'[\u3130-\u318F\uAC00-\uD7A3]+', slot_input_day.decode('utf-8')))
                if hanCount >= 1:
                    slot_input_day = self.change(slot_input_day.decode('utf-8'))
                else:
                    print('인풋 일에서 1차 분류 거치고 한글은 더이상 없는 것 같네요')
                    pass
            else:
                print('인풋 일에서는 한글은 없는 것 같네요')
                pass
            ...
            if hanCount >= 1:
                print("여기에 들어왔따")
                slot_hour = self.change_bu(re_input.decode('utf-8'))
                hanCount = len(re.findall(u'[\u3130-\u318F\uAC00-\uD7A3]+', slot_hour.decode('utf-8')))
                if hanCount >= 1:
                    slot_hour = self.change(slot_hour.decode('utf-8'))
                else:
...
#            slot_hour = int(slot_hour)
            ...
            hanCount = len(re.findall(u'[\u3130-\u318F\uAC00-\uD7A3]+', re_input.decode('utf-8')))
            if hanCount >= 1:
                slot_minute = self.change_bu(re_input.decode('utf-8'))
                hanCount = len(re.findall(u'[\u3130-\u318F\uAC00-\uD7A3]+', slot_minute.decode('utf-8')))
                if hanCount >= 1:
                    slot_minute = self.change(slot_minute.decode('utf-8'))
                else:
...
#            slot_minute = int(slot_minute)
            ...
            print ("slot_input_month : " + str(slot_input_month))
            print ("slot_input_day : " + str(slot_input_day))
            print("시이이이이이작")
...
#                else:
            try:
                if slot_input_month is not None and slot_input_month != "":
                    ...
                    #elif slot_input_day is None and slot_input_day == "":
                        next_time = next_time.replace(month=int(slot_input_month))
                else:
                    print("2")
                    if slot_input_day is not None and slot_input_day != "":
                        print("3")
                        ...
                        next_time = next_time.replace(day=int(slot_input_day))
                        print ("slot_input_day만 입력 됨" + str(next_time))
                    else:
                    #elif slot_input_day is None and slot_input_day == "":
                        print("4")
            ...
            try:
                ...
                if next_month == now_time.month:
                    if next_day == now_time.day:
                        if next_time.hour <= now_time.hour:
                            if slot_input_day is not None or slot_input_day != '':
                                day_count = 1
                            else:
                                next_time = next_time.replace(day=next_time.day + 7)
                                next_day = int(next_time.day)
                            print(next_time)
                    elif next_day < now_time.day:
                        if slot_input_day is not None and slot_input_day != '':
                            day_count = 1
                        else:
                            next_time = next_time.replace(day=next_time.day + 7)
                        ...
                        pass
                elif next_month < now_time.month:
                    hour_count = 1
                else:
                    pass
            except:
 ...
 #               next_hour = int(next_time.hour)
        ...
        if sds_res['current_task'] == 'timeAffirm':
            ...
            if day_count == 1 or month_count == 1 or hour_count == 1:
                print("dcdddddddddddddddddddddddddddddddddddd")
                print("day_count : " + str(day_count))
                print("month_count : " + str(month_count))
                print("hour_count : " + str(hour_count))
                talk.utter.utter = "$daymiss$"
                sds_res = self.Sds.Talk(talk.utter.utter, session_id, model)
                day_count = 0
                month_count = 0
                ...
                print("===============test=============")
                if (int(next_minute) == 0 or next_minute == '') or (slot_minute == 0 or slot_minute == ''):
        ...
        if sds_res['current_task'] == 'task1':
            if (sds_intent != 'affirm' and sds_intent != 'negate') and len(talk.utter.utter) >= 15:
                ...
                talk_res.response.speech.utter = "현대해상 로봇상담원입니다, "+ user_name + "고객님 되시나요?"
        ...
        if sds_res['current_task'] == 'task2':
            if sds_intent == 'again':
                talk_res.response.speech.utter = "가입하실때. 상품 내용이 제대로 설명되었는지. 확인드리고자 연락드렸습니다. 잠시 통화 가능하십니까?"
            elif sds_intent == 'unknown':
                talk_res.response.speech.utter = "네, 또는 아니오로 답변해주세요,"
            elif sds_intent == 'task2':
                talk_res.response.speech.utter = "상담사를 통해 다시 연락드리겠습니다, 현대해상 고객센터 였습니다. $callback$"
            else:
                talk_res.response.speech.utter = "네 고객님, 반갑습니다," + self.readNumberMinute(join_month) + "월" + self.readNumberMinute(join_day) +"일, 저희 현대해상 "+prod_name+"을 가입해 주셔서, 진심으로 감사드립니다. 가입하실때, 상품의 중요한 사항이 제대로 설명되었는지, 확인드리고자 연락드렸습니다, 소요시간은 약 이분정도인데. 잠시 통화 가능하십니까?"
        #PRIVACY1
        if sds_res['current_task'] == 'PRIVACY1':
            if sds_intent == 'again':
                ...
                talk_res.response.speech.utter = "죄송합니다만, 담당자와 통화 후, 다시한번 연락을 드리겠습니다, 번거롭게 해드려 죄송합니다. $callback$"
            elif sds_intent == 'privacy':
                talk_res.response.speech.utter = "죄송합니다만, 담당자와 통화 후, 다시한번 연락을 드리겠습니다, 번거롭게 해드려 죄송합니다. $callback$"
            else:
                talk_res.response.speech.utter = "지금부터 진행하는 내용은 고객님의 권리보호를 위해 녹음되며, 답변하신 내용은 향후 민원 발생시, 중요한 근거자료로 활용되오니, 정확한 답변 부탁드리겠습니다. "+ user_name + "고객님 맞으십니까"
        if sds_res['current_task'] == 'task3':
            if sds_intent == 'again':
                talk_res.response.speech.utter = "계약자와 피보험자가 다른 계약의 경우. 피보험자이신 "+ insured_person +" 고객님의 계약체결에 대한 동의가 반드시 필요합니다. 자필서명이 정확하게 이루어지지 않은 경우. 무효계약으로 고객님께서 불이익을 보실 수 있습니다. 고객님께서 청약서에 직접 자필서명을 하셨는지요?"
            elif sds_intent == 'unknown':
                talk_res.response.speech.utter = "네, 또는 아니오로 답변해주세요,"
            else:
                talk_res.response.speech.utter = "확인 감사합니다, 질문은 총 두가지입니다. 계약자와 피보험자가 다른 계약의 경우. 피보험자이신 "+ insured_person +". 고객님의 계약체결에 대한 동의가 반드시 필요합니다. 자필서명이 정확하게 이루어지지 않은 경우. 무효계약으로 고객님께서 불이익을 보실 수 있습니다. 고객님께서 청약서에 직접 자필서명을 하셨는지요?"
        # task4
        if sds_res['current_task'] == 'task4':
            if sds_intent == 'again':
                ...
                talk_res.response.speech.utter = "그러셨군요, 마지막 질문입니다. 직업, 건강상태 등 계약전 알릴의무 사항을 제대로 알리지 않으면. 향후 보험금 지급이 제한될 수 있는데요, 고객님께서 해당 내용을 정확히 확인하고 작성하셨습니까?"
        #time        
        if sds_res['current_task'] == 'time':
            if sds_intent == 'again':
                ...
                talk_res.response.speech.utter = "통화 가능하신 요일과 시를 또박또박 다시 말씀해주세요. 천구백팔십년 시월 십칠일인 경우 팔십년 시월 십칠일로 말씀해주세요."
            else:
                talk_res.response.speech.utter = "그럼 가능하신 시간을 알려주시면 다시 연락드리겠습니다, 편하신 요일과 시를, 말씀해주세요."
        #time_end       
        if sds_res['current_task'] == 'timeEnd':
            talk_res.response.speech.utter = "네 고객님, 말씀하신 시간에 다시 연락을 드리겠습니다, 현대해상 에이아이상담원 였습니다, 감사합니다. $callback$"
        #task14       
        ...
        print("시스템 대화문 : "+ talk_res.response.speech.utter)
