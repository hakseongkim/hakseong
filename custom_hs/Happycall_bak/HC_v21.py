...
#        """
        ...
        #talk.utter.utter = talk.utter.utter + ";$callSeq=6$call_Id=2007"
        ...
        if reset_count == '1':
            ...
            task_list = ['task1','task2','PRIVACY1','PRIVACY2','task3','task4','task5','task6','task7','task8','task9','task9_1']
            for i in task_list:
                seq_id_db = self.DBConnect("select seq from hc_hh_campaign_info where task = '" + i + "';")
                #print (camp_id_db)
...
#주소 입력 로직을 여기다가 수정을 해야함
        if task == 'PRIVACY1_1':
            ...
            if sds_intent == 'privacy1': 
                ...
                self.DBConnect("update task_logic set privacy1_1='"+str(privacy1_1)+"' where contract_no = '" + seq + "';")
                sds_res = self.Sds.Talk(talk.utter.utter, session_id, model)
                self.DBConnect("update hc_hh_campaign_score set task_value = 'N' where contract_no = '"+seq_id+"' and info_task = '"+ sds_res['current_task'] +"';")
            else:
                sds_res = self.Sds.Talk(talk.utter.utter, session_id, model)
...
#            if hanCount >= 1:
#                print("년 한글 확인")
#                input_year = self.change_bu(re_input)
#                hanCount = len(re.findall(u'[\u3130-\u318F\uAC00-\uD7A3]+', slot_hour.decode('utf-8')))
#                if hanCount >= 1:
#                    input_year = self.change(re_input)
#                else:
#                    print('년 한글 더이상 없음1')
#                    pass
#            else:
...
#            if hanCount >= 1:
#                print("월 한글 확인")
#                input_month = self.change_bu(re_input)
#                hanCount = len(re.findall(u'[\u3130-\u318F\uAC00-\uD7A3]+', slot_hour.decode('utf-8')))
#                if hanCount >= 1:
#                    input_month = self.change(re_input)
#                else:
#                    print('월 한글 더이상 없음1')
#                    pass
#            else:
...
#            if hanCount >= 1:
#                print("년 한글 확인")
#                input_day = self.change_bu(re_input)
#                hanCount = len(re.findall(u'[\u3130-\u318F\uAC00-\uD7A3]+', slot_hour.decode('utf-8')))
#                if hanCount >= 1:
#                    input_day = self.change(re_input)
#                else:
#                    print('월 한글 더이상 없음1')
#                    pass
#            else:
...
#                print("cust_ssn" + cust_ssn)
        task_list = ['task1','task2','task3','task4','task5','task6','task7','task8','task9_1','PRIVACY1','PRIVACY2']
        for i in task_list:
...
#            slot_minute = int(slot_minute)
            ...
            if slot_input_month is not None and slot_input_month != "":
                ...
                if slot_input_day is not None and slot_input_day != "":
                    ...
                    if slot_nextweek == "이번" or slot_nextweek == "":
        ...
        if sds_res['current_task'] == 'task1':
            if sds_intent == "again":
                talk_res.response.speech.utter = "다시 말씀드리겠습니다. 현대해상 AI상담원입니다, "+ user_name + "고객님 되시나요?"
        ...
        if sds_res['current_task'] == 'task2':
            if sds_intent == 'again':
                talk_res.response.speech.utter = "다시 한번 말씀드리겠습니다. 가입하실때, 상품 내용이 제대로 설명되었는지, 확인드리고자 연락드렸습니다, 잠시 통화 가능하십니까?"
            elif sds_intent == 'unknown':
                talk_res.response.speech.utter = "네, 또는 아니오로 답변해주세요,"
        ...
        if sds_res['current_task'] == 'PRIVACY2':
            if sds_intent == 'again':
                talk_res.response.speech.utter = "다시 한번 말씀드리겠습니다. 고객님의 주소는 " + str(privacy_add1) + ", " + str(privacy_add2) + "로 확인 되는데 맞으십니까?"
            elif sds_intent == 'unknown':
                talk_res.response.speech.utter = "네, 또는 아니오로 답변해주세요,"
