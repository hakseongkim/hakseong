...
class EchoDa(talk_pb2_grpc.DialogAgentProviderServicer):
    ...
    def Talk(self, talk, context):
        ...
        dbsession = self.DBConnect("select session_value,cust_tel_no,cust_nm,join_time,talk_time,insured_contractor,insured_person,insured_closeproduct,address_main,address_sub,product_code1, product_code2, product_code3, cust_ssn,camp_id, prod_name from campaign_target_list_tb where contract_no = '" + seq + "';")
        # cust_nm 통화자(가입자)
        # join_time 전체 시간 호출
        # join_month 가입 월
        # join_day 가입 일
...
#            session_id = int(dbsession[0]['session_value'])
        ...
        sds_data_intent = ""
        unknown_count = 0
        sds_data_intent = self.Sds.GetIntent(talk.utter.utter, model)
          ...
          ##### 한글 일 확인 
            ...
            if sds_intent == 'privacy':
                ...
                if len(input_six) <= 0 and (len(input_year) <= 0 or len(input_month) <= 0 or len(input_day) <= 0):
                    ...
                    if int(privacy1_1) >= 3:
                    #if int(privacy1_1) >= 9999:
                        talk.utter.utter = "$privacy1$"
                        sds_res = self.Sds.Talk(talk.utter.utter, session_id, model)
                    elif int(privacy1_1) <= 9998:
...
#                            sds_res = self.Sds.Talk(talk.utter.utter, session_id, model)
        ...
        if sds_res['current_task'] == 'task9':
            sds_intent = self.Sds.GetIntent(talk.utter.utter, model)
            #if sds_intent == 'affirm' or sds_intent == 'negate' or sds_intent == 'unknown' or sds_intent == 'again' or sds_intent == 'req':
            if sds_intent != 'noproportion' and sds_intent != 'proportion' and sds_intent != 'nooverlap' and sds_intent != 'overlap':
...
#                        print ("일정을 입력을 하시지 않았습니다. 다시 입력해주세요.")
        ...
        if sds_res['current_task'] == 'PRIVACY1':
            ...
            #elif int(privacy1_1) == 2 and int(privacy1_1) <= 9998:
