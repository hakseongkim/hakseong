...
#            session_id = int(dbsession[0]['session_value'])
        ...
        if task == 'PRIVACY2' and (sds_intent == "affirm" or int(privacy2_count) >= 1 or sds_intent == 'negate'):
            print("성공22222")
...
#주소 입력 로직을 여기다가 수정을 해야함
       ...
       # if sds_res['current_task'] == 'PRIVACY1_1':
        ...
        task_list = ['task1','task2','task3','task4','task5','task6','task7','task8','task9','task9_1','PRIVACY1','PRIVACY2']
        for i in task_list:
...
#            slot_minute = int(slot_minute)
        ...
        if sds_res['current_task'] == 'task4':
            if sds_intent == 'again':
                talk_res.response.speech.utter = "타인의 사망을 보장 해주는 계약의 경우 보험대상자도 반드시 서면동의를해주셔야 하는데요, 피보험자 "+ insured_person +" 님도 직접 서명하셨습니까?"
            elif sds_intent == 'unknown':
                talk_res.response.speech.utter = "네, 또는 아니오로 답변해주세요,"
            else:
